...
class LSTMFeatureScaler:
    ...
    def _safe_encode(self, value: str, encoder: LabelEncoder) -> int:
        ...
        """
        # A MISSING categorical is 'no_data' (the training fill), not 'unknown' (reserved for
        # genuinely unseen classes). An empty dynamic frame makes compute_climatology return
        # NaN for lulc; coerce None/NaN -> 'no_data' so it encodes identically to a populated
        # frame that yields the "no_data" string — keeps the fetch paths in exact parity.
