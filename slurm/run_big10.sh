...
# ─────────────────────────────────────────────
# Advisory reliability calibration (clarity constants) — NO model inference, NO artifact.
# Reuses the trained model's test predictions (${DATA_DIR}/test_samples.pkl) to compute the
# calibrated clarity + direction reliability, and prints a PASTE-READY block. A human copies
# that block into advisory/reliability.py and commits (source stays human-reviewed — a GPU run
# never rewrites code). Non-fatal: a hiccup here must not fail the training run.
# NOTE: --gwl_csv must have station_code/date/gwl_value columns (as ${CSV_PATH} does).
# ─────────────────────────────────────────────
RELIABILITY_OUT="${RUN_DIR}/reliability_constants.txt"
run_step "Reliability Calibration" "PYTHONPATH=. python scripts/calibrate_clarity.py \
    --pkl '${DATA_DIR}/test_samples.pkl' --horizon '${HORIZON}' --gwl_csv '${CSV_PATH}' \
    --state_csv '${RUN_DIR}/performance/state_performance_test.csv' --emit-constants \
    > '${RELIABILITY_OUT}' 2>&1 || echo 'reliability calibration skipped/failed (non-fatal) — see '${RELIABILITY_OUT}"
echo "Reliability constants → PASTE the block into advisory/reliability.py + commit:  ${RELIABILITY_OUT}"
