...
# NEW: tier is keyed by (water_trend = sign(fc) vs zero, vs_normal = a), NOT (a, b-yoy). So:
#   Davanagere fc=+1.54 -> falling & above -> usual-to-light  (was water-heavy: the contradiction FIX —
#     "above normal" in a DEPLETION season is losing water LESS than usual, not a surplus).
#   CG Durg fc=-6.16 -> rising & below -> usual-to-light (was water-light: rising = still GAINING water).
CASES = [
    ("SW-wet CG Durg",     -6.16, -11.71, 3.40, -0.61,  1.19, 3.09, "below",  "normal", "usual-to-light", "wet"),
    ("SW-dry Davanagere",   1.54,   9.03, 3.37,  3.28, -0.39, 4.91, "above",  "normal", "usual-to-light", "dry"),
...
def test_four_cases_regime_tier_season():
    for name, fc, ns, bs, ly, ny, by, ea, eb, etier, eseason in CASES:
        ...
        water_trend = "rising" if fc < 0 else "falling"     # absolute (vs zero) — the new tier driver
        cell = RULES[(water_trend, a)]
...
def test_long_crop_gate_asymmetric():
    ...
    # NEW gates (Step 4): water must not be falling, and an 'above' call must not be marginal.
    assert re_.long_crop_gate("above", "above", water_trend="falling")["allowed"] is False   # still drawing down
    assert re_.long_crop_gate("above", "above", water_trend="rising", clarity="moderate")["allowed"] is False  # marginal above
    assert re_.long_crop_gate("above", "above", water_trend="rising", clarity="clear")["allowed"] is True      # decisive
    assert re_.long_crop_gate("normal", "normal", water_trend="rising", clarity="typical")["allowed"] is True  # normal yr ok


def test_insufficient_data_message_uses_reason_code_and_details():
    # The decline message is built from reason_code + details (not just a generic line):
    # each code gets its own plain lead, and nearest_km / usable_years are surfaced.
    from advisory import phraser as ph
    m = ph.phrase_v0({"status": "insufficient_data", "reason_code": "sparse_history",
                      "details": {"nearest_km": 7.0, "usable_years": 1}})
    assert "enough years" in m                       # sparse_history lead
    assert "7 km" in m                               # nearest_km surfaced
    assert "1 year" in m and "1 years" not in m       # singular grammar for 1 year
    assert "KVK" in m                                # actionable next step
    # out_of_network: distance surfaced, history line absent
    m2 = ph.phrase_v0({"status": "insufficient_data", "reason_code": "out_of_network",
                       "details": {"nearest_km": 55.0}})
    assert "outside our monitored well network" in m2 and "55 km" in m2 and "year" not in m2
    # plural grammar for >1 year
    m3 = ph.phrase_v0({"status": "insufficient_data", "reason_code": "no_recent_reading",
                       "details": {"nearest_km": 4.0, "usable_years": 3}})
    assert "3 years" in m3
    # unknown/absent code -> generic fallback still renders (backward compatible), no crash
    m4 = ph.phrase_v0({"status": "insufficient_data", "reason": "some raw reason"})
    assert "Not enough nearby well data" in m4 and "some raw reason" in m4


def test_phrase_never_raises_on_malformed_advisory():
    # A phrasing bug must NOT sink an otherwise-good advisory — phrase() degrades to a safe line.
    from advisory import phraser as ph
    # type_examples=None would TypeError inside phrase_v0's join([...][:3]) -> must be caught
    m = ph.phrase({"status": "ok", "confidence": {"level": "thin"},
                   "crop_guidance": {"type_examples": None}})
    assert isinstance(m, str) and "check locally" in m.lower()
    # missing confidence entirely -> no KeyError (hardened to .get)
    m2 = ph.phrase({"status": "ok", "crop_guidance": {}})
    assert isinstance(m2, str) and m2
    # totally empty dict -> still a safe string, never a crash
    m3 = ph.phrase({})
    assert isinstance(m3, str) and m3
