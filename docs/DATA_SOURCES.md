...
| Signal | Source | How |
|---|---|---|
| **Current & historical GWL** | India‑WRIS **or** NWDP **or** a local CSV | `--gwl-source wris\|nwdp` / `--local-csv` |
...
- **India‑WRIS** (`--gwl-source wris`, default) — the official CGWB water‑resources portal.
- **NWDP** (`--gwl-source nwdp`) — a live drop‑in used when WRIS is unavailable. Finds the coordinate‑nearest well,
  applies a staleness guard, and forward‑fills (LOCF) the last reading up to the anchor.
