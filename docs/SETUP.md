...
python -m inference --run-dir $M/model_3m --prithvi-model-dir $M/prithvi_base \
    ...
    --date 2025-01-30 --gwl-source nwdp --device cuda
...
- **GWL source:** `--gwl-source nwdp` (**default** — reachable, fresher) or `wris` (official CGWB portal, but
  recurring outages make it currently unreliable). A `--local-csv` is available for offline/parity runs.
