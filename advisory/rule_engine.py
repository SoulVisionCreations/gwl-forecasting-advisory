...
def load_rules(path=None):
    """Load the decision table, keyed by (water_trend, vs_normal) -> (net_read, crop_lean, tier).
    water_trend = rising|falling (absolute, vs zero); vs_normal = above|normal|below (vs the seasonal normal)."""
    path = path or os.path.join(_RULES_DIR, "gwl_advisory_rules.csv")
    out = {}
    with open(path) as f:
        for row in csv.DictReader(f):
            out[(row["water_trend"].strip(), row["vs_normal"].strip())] = {
...
def long_crop_gate(a3, a6, water_trend=None, clarity=None):
    """Long/perennial crop allowed ONLY if ALL hold (asymmetric: easy to say no):
      - both 3m and 6m at/above normal (6m 'below' is a firm veto), AND
      - the water is NOT falling (a perennial needs the table to hold/rise, not draw down), AND
      - the 3m above-call is not MARGINAL (a barely-above 'moderate' call is too weak to commit a year+).
    """
    if a3 not in ("above", "normal") or a6 not in ("above", "normal"):
        if a6 == "below":
    ...
    if water_trend == "falling":
        return {"allowed": False,
                "reason": "the water table is still drawing down — don't commit a long crop; prefer a short one"}
    if a3 == "above" and clarity == "moderate":
        return {"allowed": False,
                "reason": "the above-normal call is only moderate — too marginal to commit a long crop; prefer a short one"}
    return {"allowed": True,
            "reason": "3m and 6m at/above normal, water not falling, call decisive — workable; keep a margin"}
