...
class TFTInferenceEngine:
    ...
    def __init__(self, cfg: InferenceConfig):
        ...
        # climatology_years defaults to the MODEL's training value (data_config) so the
        # fallback climatology matches what the model was trained on; an explicit
        # GWL_CLIMATOLOGY_YEARS env still overrides (for experiments / A-B).
        _clim_years = getattr(self.data_config, "climatology_years", None) or cfg.climatology_years
        if os.environ.get("GWL_CLIMATOLOGY_YEARS"):
            _clim_years = cfg.climatology_years
        self.builder = SampleBuilder(
            self.data_config, self.station_stats,
            climatology_years=_clim_years, use_openmeteo=cfg.use_openmeteo,
        )
