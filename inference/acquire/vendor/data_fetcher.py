...
# Hard client-side per-call timeout. A stalled getInfo/getRegion NEVER raises on this
# transport (urllib3-future), and ee.data.setDeadline does not interrupt the socket read —
# so without this a single hung call blocks the whole request indefinitely. Running fn() in
# a throwaway daemon thread and bounding it converts a hang into a retryable "timed out".
_GEE_CALL_TIMEOUT_S = int(os.environ.get("GWL_GEE_CALL_TIMEOUT_S", "60"))


def _call_with_timeout(fn, timeout_s: float):
    """Run fn() in a daemon thread and bound it. Raise RuntimeError('...timed out...') if it
    exceeds timeout_s (the orphaned thread is abandoned — a hung getInfo can't be cancelled,
    but the caller retries a fresh call). Otherwise propagate fn()'s result or exception."""
    box: dict = {}
    ...
    t.join(timeout_s)
    if t.is_alive():
        raise RuntimeError(f"GEE getInfo timed out after {timeout_s}s (abandoned, will retry)")
    if "err" in box:
        raise box["err"]
    return box.get("val")
...
def _gee_call_with_retry(fn):
    """
    Call fn() (a GEE .getInfo() lambda) with a HARD per-call timeout + exponential backoff on
    rate-limit / transient errors (the timeout counts as transient). Raises on final failure.
    """
    logger.info("GEE call initiated (max_retries=%d, timeout=%ds)",
                _GEE_MAX_RETRIES, _GEE_CALL_TIMEOUT_S)
    for attempt in range(_GEE_MAX_RETRIES):
        try:
            with _GEE_SEM:
                return _call_with_timeout(fn, _GEE_CALL_TIMEOUT_S)
        except Exception as e:
            msg = str(e).lower()
