...
DEFAULT_MAX_STALENESS_DAYS = 200
...
def resolve_anchor_gwl(gwl_df, current_date: "datetime", gap_days: int = 30,
    ...
    # NO staleness DROP: always forward-fill the last value to the anchor (ANY age). The caller
    # degrades CONFIDENCE by age_days instead of declining. (max_staleness_days kept in the
    # signature for back-compat but no longer gates.)

    # stale-but-usable: forward-fill (LOCF) daily rows from last_date+1 .. anchor.
