...
class WrisWithCsvFallbackSource:
    ...
    def __init__(self, primary, data_config, verify=False, min_rows=_MIN_ROWS,
                 ...
                 retries=_HOT_RETRIES, timeout=_HOT_TIMEOUT):
    ...
    def fetch(self, neighbours, current_date):
        ...
        for a in acquired:
            ...
            if self._down_this_fetch:                             # WRIS already failed hard this request
                if csv_n:
                    self._notes.append(f"normals: {code} WRIS down (skipped) -> CSV snapshot")
                continue
            try:
                ...
                self._notes.append(f"normals: {code} WRIS error ({type(e).__name__}) -> CSV snapshot")
                series = None
            ...
            if wris_n >= self.min_rows and (wris_n >= self.min_good_readings or wris_n >= csv_n):
                ...
                self._notes.append(f"normals: {code} from WRIS live ({wris_n} readings; csv={csv_n})")
            elif csv_n:                                           # keep the CSV snapshot (no downgrade)
                if wris_n >= self.min_rows:
                    self._notes.append(f"normals: {code} WRIS thinner ({wris_n} < csv {csv_n}) -> kept CSV")
                elif wris_n > 0:
                    self._notes.append(f"normals: {code} WRIS too few ({wris_n} < min {self.min_rows}) -> CSV")
                else:
                    self._notes.append(f"normals: {code} WRIS empty -> CSV snapshot fallback")
